...
- The live PR ingestion path can persist bounded PR metadata, changed-file metadata, public patch excerpts, reviews, and review comments, and the scoring engine can now verify deterministic score replay over selected persisted evidence without writing new score state. Profile-service can now force a persisted snapshot rebuild from stored score and badge evidence, scheduler-worker can execute that refresh as `profile.refresh_user`, and `pipeline.grade_pull_request` can chain bounded PR sync, persisted analysis, score replay, profile refresh, and live report verification for a known user and PR. Dedicated report materialization/backfill is still pending.
...
- [ ] Add an orchestration path for `sync PR -> fetch bounded files/diff features -> analyze -> persist contribution_analyses -> score -> refresh profile -> materialize PR report`. The analyzer now persists deterministic `contribution_analyses` rows for already-synced PRs and returns `analysis_id`/`pull_request_id`, scheduler-worker can execute `analysis.pull_request` jobs against that route, `profile.refresh_user` can rebuild persisted profile snapshots after scoring, and `pipeline.grade_pull_request` now chains sync, analysis, score replay, profile refresh, and live PR-report verification for a known user and PR; the remaining gap is dedicated PR-report materialization/backfill storage rather than live report rendering.
...
- [ ] Every XP value must link to a persisted score event, formula version, PR evidence, and analysis artifact. PR battle reports now expose score-event formula components for newly replayed scores, and profile score history now exposes score version, formula version, pull-request ID, analysis ID, evidence state, and missing-link reasons for each XP row. Remaining leaderboard, quest reward, and older historical-event surfaces still need full linkage.
...
- [ ] Every leaderboard rank must link to a season snapshot, rank movement event, score version, and freshness timestamp. Leaderboard rows now expose profile snapshot ID/version, score version, source watermark, rank evidence state, and missing evidence reasons, so the UI shows when ranks still lack season snapshots or rank-movement events.
