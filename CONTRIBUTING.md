...
- [ ] Add an orchestration path for `sync PR -> fetch bounded files/diff features -> analyze -> persist contribution_analyses -> score -> refresh profile -> materialize PR report`. The analyzer now persists deterministic `contribution_analyses` rows for already-synced PRs and returns `analysis_id`/`pull_request_id`, scheduler-worker can execute `analysis.pull_request` jobs against that route, and `profile.refresh_user` can rebuild persisted profile snapshots after scoring; the remaining gap is a single end-to-end job that chains sync, analysis, score replay, profile refresh, and PR-report materialization.
