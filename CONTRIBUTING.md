...
- [ ] Every XP value must link to a persisted score event, formula version, PR evidence, and analysis artifact. PR battle reports now expose score-event formula components for newly replayed scores, quest XP rewards now create `quest.reward` score events linked to completion evidence, quest reward writes now backfill linked pull-request and analysis IDs from the supporting score events when available, profile score history now exposes score version, formula version, pull-request ID, analysis ID, evidence state, and missing-link reasons for each XP row (including quest rewards alongside the latest replay run), and leaderboard entries now expose rank-linked score event, formula version, pull-request ID, and analysis ID fields derived from persisted score history evidence. Remaining older historical-event surfaces still need full linkage.
