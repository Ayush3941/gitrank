...
- Reworded public profile route-level empty and error states around published scored evidence and moved fallback recovery to the public leaderboard instead of the authenticated dashboard.
...
- Normalized public-profile empty states to evidence-specific labels and descriptions for repositories, skills, badges, timeline, and battle reports.
...
- Routed unlocked badge earned-date copy through a badge-specific helper so missing timestamps read `Earned date pending` instead of `Earned Never`.
...
- Fixed the shared `Progress` primitive so caller-provided ARIA props reach the Radix progressbar root, restoring accessible names across dashboard, quest, badge, and onboarding meters.
...
- Split quest mission rendering into `QuestsMissionsSection`, moving mission loading/error/empty states, lazy `QuestCard` loading, group headings, and show-more controls out of `QuestsPageClient` while keeping quest filters and visible-count state in the page.
...
- Split settings sync-run filter rendering into `SyncRunActivityFilters`, moving search, status segmented control, live result count, and reset action out of `SyncRunActivityPanel`; updated the segmented-filter guard to audit the extracted filter component.
