...
- [ ] Apply and verify live GitHub repository controls before V2 release branches are cut. `.github/workflows/verify-live-v2-gates.yml` can run `make verify-github-repository-controls` with `GITRANK_REPO_ADMIN_TOKEN`, but the live setting mutation still needs explicit apply.
- [ ] Run and record staging rollback and restore drills.
- [ ] Replace provider-neutral Kubernetes placeholders with environment-specific secrets, TLS, ingress, managed PostgreSQL, managed Redis, registry, and environment-tuned autoscaling thresholds. `make render-k8s-release-manifests` now enforces runtime override injection and placeholder rejection for release manifests, and `.github/workflows/verify-live-v2-gates.yml` can run that gate in CI, but live environment values and rollout proof are still required.
