...
- Frontend effective sync-state precedence now only treats `running`/`queued` runs as active when they appear before the newest terminal run in reverse-chronological history, preventing stale historical active rows from overriding a newer `completed`/`partial`/`failed` status.
...
- `POST /v1/sync/user/execute` now requires a valid linked GitHub OAuth user token and no longer silently falls back to installation/shared clients for user-history discovery. Missing or undecryptable OAuth tokens now return explicit auth-required failure so the UI cannot report a misleading synced state with empty PR evidence.
