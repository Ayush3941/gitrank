...
func TestReviewSyncExecutionRequiresExecutor(t *testing.T) {
	router := NewRouter(testConfig(), testLogger(), "test")
	request := httptest.NewRequest(http.MethodPost, "/v1/sync/review/execute", bytes.NewReader([]byte(`{"repository":"octo/repo","number":7}`)))
	request.Header.Set("Content-Type", "application/json")
	response := httptest.NewRecorder()
