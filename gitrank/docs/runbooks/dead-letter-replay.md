...
| PR grading pipeline | `pipeline.grade_pull_request` scheduler job | Replay only after confirming the GitHub token, analyzer store, scoring store, profile store, and report read model are healthy. | The job has a user-plus-PR dedupe key and each stage keeps its own idempotency guard. |
...
6. Verify the downstream artifact for the stage:
   ...
   - PR grading pipeline: scheduler run output includes `grade.sync`, `grade.analysis`, `grade.score_replay`, `grade.profile_refresh`, and `grade.report`
