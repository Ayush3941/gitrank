...
if [ -f "$live_env_template" ]; then
  ...
  printf 'bootstrap hint: make -C %s scaffold-v2-live-env && edit required values in .env.v2-live-gates.local, then run CONFIRM_FINALIZE_V2=yes make -C %s finalize-v2-live-closeout-local-env\n' "$root_dir" "$root_dir"
  if [ -n "$audit_report_file" ]; then
    printf '%s\n' "- bootstrap hint: make -C $root_dir scaffold-v2-live-env then CONFIRM_FINALIZE_V2=yes make -C $root_dir finalize-v2-live-closeout-local-env" >>"$audit_report_file"
  fi
fi
