...
fail() {
  printf 'observability evidence verification failed: %s\n' "$1" >&2
  exit 1
}
...
[ -n "$record_file" ] || fail "usage: verify_observability_evidence_record.sh <record-file>"
...
printf 'observability evidence verification passed\n'
printf 'record: %s\n' "$record_file"
