...
fail() {
  printf 'rollback drill evidence verification failed: %s\n' "$1" >&2
  exit 1
}
...
[ -n "$record_file" ] || fail "usage: verify_rollback_drill_record.sh <record-file>"
...
printf 'rollback drill evidence verification passed\n'
printf 'record: %s\n' "$record_file"
