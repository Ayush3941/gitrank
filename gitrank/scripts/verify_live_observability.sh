...
EXPECTED_DASHBOARD_TITLES="${EXPECTED_DASHBOARD_TITLES:-GitRank Overview,GitRank SLOs}"
TMP_ROOT="${TMPDIR:-/tmp}"
...
curl -fsS "$PROMETHEUS_BASE_URL/-/ready" >"$prom_ready_file"
grep -qi "ready" "$prom_ready_file" || fail "Prometheus readiness endpoint did not report ready"

curl -fsS "$PROMETHEUS_BASE_URL/api/v1/targets" >"$prom_targets_file"
...
curl -fsS "$PROMETHEUS_BASE_URL/api/v1/rules" >"$prom_rules_file"
...
curl -fsS -G "$PROMETHEUS_BASE_URL/api/v1/query" --data-urlencode 'query=count({__name__=~"gitrank_.*"})' >"$prom_query_file"
...
curl -fsS -H "Authorization: Bearer $GRAFANA_API_TOKEN" "$GRAFANA_BASE_URL/api/search?type=dash-db&query=GitRank" >"$grafana_search_file"
