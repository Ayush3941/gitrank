...
- PR report pages now use a live read model, but the report still depends on analysis and score events already being persisted; exact scorer-component materialization, badge-unlock details, and full pipeline orchestration remain pending. Quests now use a live profile-owned read model, but persistent quest definitions, assignments, completion ledgers, reward grants, and audit records are still pending
