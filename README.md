...
- Settings now avoids duplicated install prompts in blocked App-sync states: one install CTA remains in the sync-block notice, while account action rows stay focused on session/account actions.
...
- Profile refresh now returns `pending_score_evidence` and skips publishing a new profile snapshot when score replay produced zero score rows; sync telemetry records `post_sync_profile_refresh_pending_score_evidence` so empty evidence cannot be reported as a healthy refresh.
- Contribution-card projection no longer sums linked score-history rows for the same PR. PR-facing XP uses the canonical `score.computed` event/report row, while quest reward evidence can still be shown as supporting context.
- Backend AI provider resolution is strict when `AI_PROVIDER` is set: generic `AI_*` overrides still work, but `openai` never borrows `GEMINI_*`, `gemini` never borrows `OPENAI_*`, and Gemini fallback is allowed only when no provider is selected and Gemini is the only configured key.
