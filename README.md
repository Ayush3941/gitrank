...
- no external worker backend yet; scheduler-worker persistent mode now uses dedicated PostgreSQL tables for jobs, dead letters, backfill plans, rate-limit windows, and scheduler counters, but only bounded sync, PR-analysis, score-replay, profile-refresh, PR-report materialization, leaderboard materialization, and PR-grading pipeline jobs currently auto-execute inside `scheduler-worker`
