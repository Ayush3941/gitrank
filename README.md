...
- no dedicated normalized job-table worker backend yet; scheduler-worker persistent mode shares runtime state through a single PostgreSQL JSONB row, and only bounded `sync.installation`, `sync.repository`, owned-repository `sync.user_history`, direct `sync.pull_request`, PR-surface `sync.review`, direct `sync.issue`, and direct `sync.commit` jobs currently auto-execute inside `scheduler-worker`
