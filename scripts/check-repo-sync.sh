...
assert_frontend_button_accessible_names() {
  if ! (
    cd "$ROOT_DIR/frontend" && node scripts/check-button-accessible-name.mjs >/dev/null
  ); then
    fail "frontend button accessible-name guard failed"
  fi
}

assert_frontend_link_accessible_names() {
  if ! (
    cd "$ROOT_DIR/frontend" && node scripts/check-link-accessible-name.mjs >/dev/null
  ); then
    fail "frontend link accessible-name guard failed"
  fi
}
...
assert_frontend_focus_without_scroll() {
  if ! (
    ...
    fail "frontend focus-without-scroll guard failed"
  fi
}
