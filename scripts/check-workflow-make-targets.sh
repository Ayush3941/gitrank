...
fail() {
  printf 'workflow make target check failed: %s\n' "$1" >&2
  exit 1
}
...
while IFS= read -r match; do
  ...
  # format: path:line:matched-command
  file="${match%%:*}"
  rest="${match#*:}"
  line_no="${rest%%:*}"
  command_line="${rest#*:}"
  ...
  # If make explicitly changes directory, only validate gitrank-scoped commands.
  if [[ -n "$cdir" ]]; then
  ...
  cd "$ROOT_DIR" && rg -n -o \
    '([A-Z_][A-Z0-9_]*=[^[:space:]]+[[:space:]]+)*make([[:space:]]+-C[[:space:]]+[^[:space:]]+)?[[:space:]]+[A-Za-z0-9_.-]+' \
    .github/workflows --glob '*.yml' --glob '*.yaml' | sort -u
)

if [[ "$missing" -ne 0 ]]; then
  fail "workflow make target references are out of sync with gitrank/Makefile"
fi

printf 'workflow make target check passed\n'
