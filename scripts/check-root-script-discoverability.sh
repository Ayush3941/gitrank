...
fail() {
  printf 'root script discoverability check failed: %s\n' "$1" >&2
  exit 1
}
...
mapfile -t tracked_scripts < <(cd "$ROOT_DIR" && git ls-files 'scripts/*.sh' | sort)
if [[ "${#tracked_scripts[@]}" -eq 0 ]]; then
  fail "no tracked scripts/*.sh files found"
fi
...
if [[ "${#queue[@]}" -eq 0 ]]; then
  fail "no root scripts are reachable from docs/repo-sync entrypoints"
fi
...
for script_path in "${tracked_scripts[@]}"; do
  ...
  if [[ -z "${reachable[$normalized]+1}" ]]; then
    printf 'unreachable root script (no entrypoint path): %s\n' "$script_path" >&2
    missing=1
  fi
done

if [[ "$missing" -ne 0 ]]; then
  fail "one or more root scripts are not discoverable from entrypoints"
fi

printf 'root script discoverability check passed\n'
