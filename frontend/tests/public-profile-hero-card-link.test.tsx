...
describe("public profile hero", () => {
  ...
  it("keeps dashboard and public hero icons decorative", () => {
    const user = buildHeroUser();
    const publicHero = render(
      <PublicProfileHero
        user={user}
        shareHeadline="Share headline"
      />,
    );
    expect(publicHero.container.querySelectorAll("svg:not([aria-hidden='true'])")).toHaveLength(0);
    publicHero.unmount();

    const dashboardHero = render(<DashboardHeroRankCard user={user} />);
    expect(dashboardHero.container.querySelectorAll("svg:not([aria-hidden='true'])")).toHaveLength(0);
  });
});

function buildHeroUser(): UserProfile {
  return {
    ...
    gitRankScore: 1337,
    consistencyScore: 84,
    strongestSignals: ["Backend", "Testing"],
