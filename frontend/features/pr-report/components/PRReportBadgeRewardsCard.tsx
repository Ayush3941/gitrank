...
export function PRReportBadgeRewardsCard({
  ...
  const badgeCountLabel = formatPluralCount(badges.length, "badge reward");
  const rewardStatusLabel =
    badges.length === 1 ? "Reward unlocked" : `${badgeCountLabel} unlocked`;

  return (
    <section className="render-opt-section">
      <div className="space-y-4">
        <h2 className="text-sm font-semibold text-white">
          Badge rewards <span className="text-muted">({badgeCountLabel})</span>
        </h2>
