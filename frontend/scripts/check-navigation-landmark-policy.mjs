...
function hasAccessibleName(attributes) {
  const ariaLabel = readStringAttribute(attributes, "aria-label");
  if (ariaLabel && ariaLabel.trim()) {
    return true;
  }
  const labelledBy = readStringAttribute(attributes, "aria-labelledby");
  if (labelledBy && labelledBy.trim()) {
    return true;
  }
  return hasNonEmptyExpressionAttribute(attributes, "aria-labelledby");
}
...
function hasNonEmptyExpressionAttribute(attributes, name) {
  for (const attribute of attributes) {
    if (
      attribute?.type !== "JSXAttribute" ||
      attribute.name?.type !== "JSXIdentifier" ||
