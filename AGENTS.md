...
## Framework Notice
- This repo uses a modern Next.js/App Router stack where APIs and conventions can shift across versions.
- Before adding or changing framework behavior, check the locally installed framework docs in `node_modules/next/dist/docs/` and prefer current project patterns over memory.
...
8. Finish goals loudly.
   - Only trigger the repository alarm when the active goal is actually complete, immediately before the final goal-completion update.
   - Do not trigger the alarm for intermediate exploration, mid-task checks, progress updates, routine verification runs, or per-slice completion updates unless explicitly requested.
   - Prefer the standard repo path: `alarm/notify.sh`.
   - Shared scripts such as `scripts/review.sh` must not ring automatically; the agent owns the final two-ring goal-completion signal.
...
5. If and only if the active goal is complete, trigger the repository alarm twice immediately before the goal-completion update.
6. Update `CONTRIBUTING.md`.
7. If any temporary hardcode/stub remains, disclose it in `README.md`.
8. Self-review the diff for correctness, regressions, and polish.
...
- After the final automated run for a completed goal:
  - confirm the repository alarm fired twice through the standard notifier path
